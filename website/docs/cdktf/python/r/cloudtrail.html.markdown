...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        aws_s3_bucket_foo = aws.s3_bucket.S3Bucket(self, "foo",
            bucket="tf-test-trail",
            force_destroy=True
        )
        data_aws_caller_identity_current =
        aws.data_aws_caller_identity.DataAwsCallerIdentity(self, "current")
        data_aws_iam_policy_document_foo =
        aws.data_aws_iam_policy_document.DataAwsIamPolicyDocument(self, "foo_2",
            statement=[DataAwsIamPolicyDocumentStatement(
                ...
                resources=[cdktf.Token.as_string(aws_s3_bucket_foo.arn)],
                ...
                resources=["${" + aws_s3_bucket_foo.arn + "}/prefix/AWSLogs/${" + data_aws_caller_identity_current.account_id + "}/*"
                ],
        ...
        aws.cloudtrail.Cloudtrail(self, "foobar",
            include_global_service_events=False,
            name="tf-trail-foobar",
            s3_bucket_name=cdktf.Token.as_string(aws_s3_bucket_foo.id),
            s3_key_prefix="prefix"
        )
        aws_s3_bucket_policy_foo = aws.s3_bucket_policy.S3BucketPolicy(self, "foo_4",
            bucket=cdktf.Token.as_string(aws_s3_bucket_foo.id),
            policy=cdktf.Token.as_string(data_aws_iam_policy_document_foo.json)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_s3_bucket_policy_foo.override_logical_id("foo")
```
