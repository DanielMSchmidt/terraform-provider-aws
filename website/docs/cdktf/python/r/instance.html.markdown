...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        aws_vpc_my_vpc = aws.vpc.Vpc(self, "my_vpc",
            cidr_block="172.16.0.0/16",
            tags={
                "Name": "tf-example"
            }
        )
        aws_subnet_my_subnet = aws.subnet.Subnet(self, "my_subnet",
            availability_zone="us-west-2a",
            cidr_block="172.16.10.0/24",
            tags={
                "Name": "tf-example"
            },
            vpc_id=cdktf.Token.as_string(aws_vpc_my_vpc.id)
        )
        aws_network_interface_foo = aws.network_interface.NetworkInterface(self, "foo",
            private_ips=["172.16.10.100"],
            subnet_id=cdktf.Token.as_string(aws_subnet_my_subnet.id),
            tags={
                "Name": "primary_network_interface"
            }
        )
        aws_instance_foo = aws.instance.Instance(self, "foo_3",
            ...
            network_interface=[InstanceNetworkInterface(
                device_index=0,
                network_interface_id=cdktf.Token.as_string(aws_network_interface_foo.id)
            )
            ]
        )
...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_aws_ami_amzn_linux2023_ami = aws.data_aws_ami.DataAwsAmi(self, "amzn-linux-2023-ami",
            filter=[DataAwsAmiFilter(
                name="name",
        ...
        aws_instance_example = aws.instance.Instance(self, "example_3",
            ami=cdktf.Token.as_string(data_aws_ami_amzn_linux2023_ami.id),
            cpu_options=InstanceCpuOptions(
                core_count=2,
                threads_per_core=2
            ),
            instance_type="c6a.2xlarge",
            subnet_id=cdktf.Token.as_string(aws_subnet_example.id),
            tags={
                "Name": "tf-example"
            }
        )
...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        aws.instance.Instance(self, "this",
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-2ab1cd7123c9251b15c0901dbc68b91485aedc2ad964c9969b8c1365190ba082 -->