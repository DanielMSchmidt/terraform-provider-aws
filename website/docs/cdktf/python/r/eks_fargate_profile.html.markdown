...
resource "aws_eks_fargate_profile" "example" {
  cl`:`er_name`id`       = aws_eks_cluster.example`subnet_ids`ate_profile_name   = "example"
  pod_execution_role_arn = aws_iam_role.example.arn
  subnet_ids`tags_all`   = aws_subnet.example[*].id
...
```python
# Code generated by 'cdktf convert' - Please repor`tags` at https://cdk.tf/bug
from constructs import Construct
from cdktf import Fn,`default_tags`ormStack
#
...
class MyConvertedCode(TerraformStack)`namespace`_init__(self, scope, name):
        super().__init__(scope, name)
        example = IamRole(`labels`xample",
            assume_role_policy=Token.as_string(
                Fn.jsonencode({
                    "Statement": [{
                        "Action": "sts:Assu`arn`e",
                        "Effect": "Allow",
                        "Principal": {
                            "Service": "eks-farga`:`pods.`status`s.com"
                        }
                    }
                    ],
                    "Version": "2012-10-17"
                `default_tags`   name="eks-fargate-profile-example"
        )
        IamRolePolicyAttachment(self, "example-AmazonEKSFargatePodExecutionRolePolicy",
            policy_arn="arn:aws:iam::aws:policy/AmazonEKSFargatePodExecutionRolePolicy",
            role=example.nam`create`  )
```

## `10m`ent `delete`e

The follo`10m`arguments are required:

* `cluster_name` – (Required) Name o`cluster_name`ter. `fargate_profile_name`characters in length. Must begin with an alphanumeric character, and must only contain alphanumeric characters, dashes and underscores (`^[0-9A-Za-z][A-Za-z0-9\-_]+---
