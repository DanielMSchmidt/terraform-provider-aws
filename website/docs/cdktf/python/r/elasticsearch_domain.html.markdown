...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        aws.elasticsearch_domain.ElasticsearchDomain(self, "example",
            access_policies="{\n  \"Version\": \"2012-10-17\",\n  \"Statement\": [\n    {\n      \"Action\": \"es:*\",\n      \"Principal\": \"*\",\n      \"Effect\": \"Allow\",\n      \"Resource\": \"arn:aws:es:${" + data_aws_region_current.name + "}:${" + data_aws_caller_identity_current.account_id + "}:domain/${" + domain.value + "}/*\",\n      \"Condition\": {\n        \"IpAddress\": {\"aws:SourceIp\": [\"66.193.100.22/32\"]}\n      }\n    }\n  ]\n}\n",
...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        aws_iam_service_linked_role_es =
        aws.iam_service_linked_role.IamServiceLinkedRole(self, "es",
        ...
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        data_aws_region_current.override_logical_id("current")
        data_aws_vpc_selected = aws.data_aws_vpc.DataAwsVpc(self, "selected",
            tags={
                "Name": vpc.string_value
            }
        )
        aws_security_group_es = aws.security_group.SecurityGroup(self, "es_6",
            description="Managed by Terraform",
            ingress=[SecurityGroupIngress(
                cidr_blocks=[cdktf.Token.as_string(data_aws_vpc_selected.cidr_block)],
                from_port=443,
                protocol="tcp",
                to_port=443
            )
            ...
            vpc_id=cdktf.Token.as_string(data_aws_vpc_selected.id)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_security_group_es.override_logical_id("es")
        data_aws_subnets_selected = aws.data_aws_subnets.DataAwsSubnets(self, "selected_7",
            filter=[DataAwsSubnetsFilter(
                name="vpc-id",
                values=[cdktf.Token.as_string(data_aws_vpc_selected.id)]
            )
            ],
            tags={
                "Tier": "private"
        ...
        aws.elasticsearch_domain.ElasticsearchDomain(self, "es_8",
            access_policies="{\n\t\"Version\": \"2012-10-17\",\n\t\"Statement\": [\n\t\t{\n\t\t\t\"Action\": \"es:*\",\n\t\t\t\"Principal\": \"*\",\n\t\t\t\"Effect\": \"Allow\",\n\t\t\t\"Resource\": \"arn:aws:es:${" + data_aws_region_current.name + "}:${" + data_aws_caller_identity_current.account_id + "}:domain/${" + domain.value + "}/*\"\n\t\t}\n\t]\n}\n",
            advanced_options={
            ...
            depends_on=[aws_iam_service_linked_role_es],
            domain_name=domain.string_value,
            ...
            vpc_options=ElasticsearchDomainVpcOptions(
                security_group_ids=[cdktf.Token.as_string(aws_security_group_es.id)],
                subnet_ids=[
                    cdktf.Token.as_string(
                        cdktf.property_access(data_aws_subnets_selected.ids, ["0"])),
                    cdktf.Token.as_string(
                        cdktf.property_access(data_aws_subnets_selected.ids, ["1"]))
                ]
            )
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-faeaf2d63f5799fcc0ac8cdd28aea3bc41938ded47a07d926bf6581853886513 -->