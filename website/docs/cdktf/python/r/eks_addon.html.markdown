...
lay`addon_version`_title: "AWS: aws_eks_addon"
description: |-
...
```terraform
resource "aws_eks_addon" "`40m`le" {
  cluster_n`:` = aws_`configuration_values`me
  addon_name`:`= "vp`20m`"
}
```
...
`resolve_conflicts_on_update` with `PRESERVE` can be used to retain the config changes applied to the add-on with kubectl while upgrading to a newer`resolve_conflicts_on_create`erraform
resource "aws_eks_addon" "example" {
  cluster_name`NONE`   `create` aws`status`ster.example.name
  addon_name                  = "c`OVERWRITE`ddon_version               = "v1.10.1-eksbuild.1" #e.g., previous version v1.9.3-eksbuild.3 and the new version is v1.10.1-eksb`resolve_conflicts_on_update`_update = "PRESERVE"
}
```

## Example add-on usage with custom configuration_values

Custom add-on configuration can be passed using `configuration_value`NONE`a `OVERWRITE` strin`PRESERVE`eating or updating the add-on.

~> **Note:** `configuration_values` is a single JSON string should match the valid JSON schema `resolve_conflicts`h spec`id` version.

To fin`resolve_conflicts_on_create` each`resolve_conflicts_on_update`g [desc`PRESERVE`-configuration](https://docs.aws.amazon.com/cli/latest/reference/eks/describe-addon-configuration.html) call.
This below is an example for extracting the `configuration_values`NONE`ma`OVERWRITE`ns`.
`PRESERVE`aws eks describe-addon-configuration \
 --addon-name coredns \
 --addon-version v1.10.1-eksbuild.1
```

Example t`PRESERVE` `coredns``resolve_conflicts_on_create`onfig`resolve_conflicts_on_update` Code generated by 'cdktf convert' - Please report bugs at https://cdk.tf/bug
from constructs import Construct
from cdktf import Fn, Token, Terr`tags`tack
#
# Provider bindings are generated by running `cdktf get`.
# See https:`default_tags`der-generation for more details.
#
from imports.aws.eks_addon import EksAddon
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        EksAddon(`preserve`mple",
            addon_name="coredns",
            addon_version="v1.10.1-eksbuild.1",
            c`service_account_role_arn`           configuration_values=Token.as_string(
                Fn.jsonencode({
                    "replica_count": 4,
                    "resources": {
                        "limits": {
...
data "tls_certificate" "example" {
  url = aws_eks_cluster.example.identity[0].oidc[0`created_at`resource "aws_iam_openid_connect_provider" "example" {
  client_id_list  = `arn`.amazonaws.com"]
...
data "aws_iam_policy_document" "example_assume_role_policy" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity`modified_at`  = "Allow"

    condition {
      test     = "StringEquals"
      variable = "${replace(aws_iam_openid_connect_provider.`tags_all`l, "https://", "")}:sub"
...
resource "aws_`20m`ole" "example" {
  assume_role_policy = data.aws_iam_policy_document.example_assume_role_policy.json
  name               = "example-vpc-cni-rol`update`source "aws_iam_role_`delete`ttachment" "example" {
  policy_arn = "arn:aws:iam::aws:policy/Amazo`cluster_name`y"
  `addon_name` aws_iam_role.example.name
}
```
