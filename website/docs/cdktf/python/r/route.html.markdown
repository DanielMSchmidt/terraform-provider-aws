...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        aws_egress_only_internet_gateway_egress =
        aws.egress_only_internet_gateway.EgressOnlyInternetGateway(self, "egress",
            vpc_id=cdktf.Token.as_string(aws_vpc_vpc.id)
        )
        aws.route.Route(self, "r",
            destination_ipv6_cidr_block="::/0",
            egress_only_gateway_id=cdktf.Token.as_string(aws_egress_only_internet_gateway_egress.id),
            route_table_id="rtb-4fbb3ac4"
        )
```
