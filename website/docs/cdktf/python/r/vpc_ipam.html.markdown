...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        aws.vpc_ipam.VpcIpam(self, "main",
            description="My IPAM",
...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        ipam_regions = cdktf.TerraformVariable(self, "ipam_regions",
            default=["us-east-1", "us-west-2"],
            type=cdktf.VariableType.ANY
        )
        data_aws_region_current = aws.data_aws_region.DataAwsRegion(self, "current")
        all_ipam_regions = cdktf.Fn.distinct(
            cdktf.Token.as_any(
                cdktf.Fn.concat([[data_aws_region_current.name], ipam_regions.value])))
        # In most cases loops should be handled in the programming language context and
        #     not inside of the Terraform context. If you are looping over something external, e.g. a variable or a file input
        #     you should consider using a for loop. If you are looping over something only known to Terraform, e.g. a result of a data source
        #     you need to keep this like it is.
        aws_vpc_ipam_main_dynamic_iterator0 = cdktf.TerraformIterator.from_list(
            cdktf.Token.as_any(all_ipam_regions))
        aws.vpc_ipam.VpcIpam(self, "main",
            description="multi region ipam",
            operating_regions=aws_vpc_ipam_main_dynamic_iterator0.dynamic({
                "region_name": aws_vpc_ipam_main_dynamic_iterator0.value
            })
        )
```
