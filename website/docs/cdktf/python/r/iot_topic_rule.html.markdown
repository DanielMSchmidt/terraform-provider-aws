...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        aws_sns_topic_myerrortopic = aws.sns_topic.SnsTopic(self, "myerrortopic",
            name="myerrortopic"
        )
        aws_sns_topic_mytopic = aws.sns_topic.SnsTopic(self, "mytopic",
            name="mytopic"
        )
        ...
        data_aws_iam_policy_document_iam_policy_for_lambda =
        aws.data_aws_iam_policy_document.DataAwsIamPolicyDocument(self, "iam_policy_for_lambda",
            statement=[DataAwsIamPolicyDocumentStatement(
                actions=["sns:Publish"],
                effect="Allow",
                resources=[cdktf.Token.as_string(aws_sns_topic_mytopic.arn)]
            )
            ]
        )
        ...
        aws_iam_role_policy_iam_policy_for_lambda =
        aws.iam_role_policy.IamRolePolicy(self, "iam_policy_for_lambda_5",
            name="mypolicy",
            policy=cdktf.Token.as_string(data_aws_iam_policy_document_iam_policy_for_lambda.json),
            role=cdktf.Token.as_string(aws_iam_role_role.id)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        aws_iam_role_policy_iam_policy_for_lambda.override_logical_id("iam_policy_for_lambda")
        aws.iot_topic_rule.IotTopicRule(self, "rule",
            description="Example rule",
            enabled=True,
            error_action=IotTopicRuleErrorAction(
                sns=IotTopicRuleErrorActionSns(
                    message_format="RAW",
                    role_arn=cdktf.Token.as_string(aws_iam_role_role.arn),
                    target_arn=cdktf.Token.as_string(aws_sns_topic_myerrortopic.arn)
                )
            ),
            name="MyRule",
            sns=[IotTopicRuleSns(
                message_format="RAW",
                role_arn=cdktf.Token.as_string(aws_iam_role_role.arn),
                target_arn=cdktf.Token.as_string(aws_sns_topic_mytopic.arn)
            )
            ],
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-9808ad0073de832344857ed487ed5aa618aec4545b697b3cfbba05f6d5614832 -->