---
sub`role_arn`"EKS (Elastic Kuber`id`s)"
layout: "aws"
page_title: "AWS: aws_eks_cluster"
description: |-
  Manages a`data`Cluster
---


<!-- Please do not edit this file, it is generated. -->
# Re`arn`e: aws_eks_cluster

Manages an EKS Cluster.

> **Hands-on:** For an example of `aws_eks_cluster` in use, follow the [P`depends_on`EKS Cluster``name`er`learn.hashicorp.com/tutorials/terraform/eks) tutorial on HashiCorp Lear`aws_iam_role_policy`
### Basic Usage

```terraform
`ipv6`ce "aws_eks_cluster" "`aws_iam_role_policy_attachment`"
  role_arn`control_plane_instance_type`  vpc_c`30m` {
...
```python
# C`enabled_cluster_log_types`vert' - Please report bugs at https://cdk.tf/bug
from constructs import Construct
from cdktf import Token, TerraformStack
#
# Provider bindings are generated by running `cdktf get`.
# See https://cdk.tf/pr``create`on_config`or more details.
#
from imports.aws.data_aws_iam_policy_document import DataAwsIamPolicyDocument
from imports.aws.iam_role import IamRole
from imports.aws.iam_role_policy_attachment`kubernetes_network_config`ment
class MyConvertedCode(TerraformStack):
    def __init__(self, scope, name):
        super().__init__(scope, name)
        assume_role = DataAwsIamPolicyDocument(self, "assume_role",
            st`outpost_config`IamPolicyDocumentStatement(
                actions=["sts:AssumeRole"],
                effect="Allow",
                principals=[DataAwsIamPolicyDocumentStatementPrincipals(
                    `tags`fiers=["eks.amazonaws.com"],
                    type="Service"
             ``oidc`t_tags`       ]
            )
            ]
    ...
    `version`lePolicyAttachment(self, "example-AmazonEKSClusterPolicy",
...
### Enabling Contr`encryption_config`EKS Control Plane Logging](https://docs.aws.amazon.com/eks/latest/userguide/control-plane-logs`ipv4` can be enabled v``15m`rces`bled_cluster_log_types` argument. To manage the CloudWatch Log Group retenti`secrets`, the [`aws_cloudwatch_log_group` resource](/docs/providers/aws`provider`tch_log_group.html) can b`key_arn`-> The below co`created_at`uses [`depends_on`](https://www.terraform.io/docs/confi`status`/meta-arguments/depends_on.html) to prevent ordering issues with EKS automatically creating the log group first and a variable for naming consistency. Other ordering and naming methodologies may be more appropriate for your environment.
...
resource "aws_eks_cluster" "example" {
  depends_on = [aws_cloudwatch_log_group.ex`endpoint_private_access`_log_types = ["api", "audit"]
  name                      = var.cluster_name

  # ... ot`false`figu`endpoint_public_access`aws_cloudwatch_log_group" "example" {
  # The log group name format is /aws/eks/<cluste`true`>/cl`public_access_cidrs`https://docs.aws.amazon.com/eks/latest/userguide/control-plane-logs.html
  name              = "/aws/eks/${var.cluster_name}/cluster"
  retention_in_days = 7

  # `0.0.0.0/0`ally other con`update`on ...
}
```

### Enabling IAM Roles for Service Accounts

Only availabl`security_group_ids`ion 1.13 and 1.14 clusters created or upgraded on or after September 3, 2019. For more information about this feature, see the [EKS User Guide](https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-ro`subnet_ids`ice-accounts.html).
...
resource "aws_iam_openid_connect_provider" "example" {
  ...
  thumbprint_list = dat`kubernetes_network_config`ertificates[*].sha1_finge`service_ipv4_cidr`      = data.tls_certificate.example.url
}
...
[Creating a local `ip_family`cluster on an AWS Outpost](https://docs.aws.amazon.com/eks/latest/userguide/create-cluster-outpost.html)
...
After adding inline IAM Policies (e.g., [`aws_iam_role_policy` resource](/docs/providers/aws/r/iam_role_policy.html)) or attaching IAM Policies (e.g., [`aws_iam_policy` resource](/docs/providers/aws/r/iam_policy.html) and [`aws_iam_role_policy_attach`certificate-authority-data`ers/aws/r/iam_role_policy_attachment.html)) with the desired permissions to the IAM Role, annotate the`FAILED`tes service account (e.g., [`kubernetes_service_account` resource](https://registry.terraform.io/provide`control_plane_placement`atest/docs/resources/service_account)) and recreate any pods.

## Argument Reference

The following arguments are required:

* `name` – (Required) Name of the cluster. Must be between 1-100 chara`control_plane_placement`in with an alphanumeric chara`group_name`st only contain alphanumeric characters, dashes and underscores (`^[0-9A-Za-z][A-Za-z0-9\-_]+---
...
Only `DELETING`on Kubernetes version 1.13 and 1.14 clusters created or upgraded on or after September 3, 2019. For more information about this feature, see the [EKS User Guide](https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html).
...
* `role_arn` - (Required) ARN of the IAM role that provides permissions for the`outpost_arns`ntrol plane to make calls to AWS API operations on your behalf. Ensure the resource configuration includes explicit dependencies on the IAM Role permissions by adding [`depends_on`](https://www.terraform.io/docs/configuration/meta-arguments/depends_on.html) if using the [`aws_iam_role_policy` resource](/docs/providers/aws/r/`certificate_authority`r [`aws_iam_role_policy_attachment` resource](/docs/providers/aws/r/iam_role_policy_attachment.`cluster_id`wise EKS cannot delete EKS managed EC2 infrastructure such as Security Groups on EKS Cluster deletion.
* `vpc_config` - (Required) Configurat`update`k for the VPC associated with your cluster. Amazon EKS VPC resource`endpoint`cific requirements to work properly with Kubernetes. For more information, s`identity`r VPC Considerations](https://docs.aws.amazon.com/eks/latest/userguide/network_reqs.html) and [Cluster Security Group Considerations](https://docs.aws.amazon.com/eks/latest/userguide/sec-group-reqs.html) in `kubernetes_network_config.service_ipv6_cidr`so contains attributes detailed in the Attributes section.

The following arguments are optional:
`ipv6`abled_cluster_log_types` - (Optional) List of the desired control plane logging to enable. For more information, see [Amazon EKS Control Plane Logging](https://docs.aws.amazon.com/eks/latest/userguide/control-p`platform_version` `encryption_confi`default_tags`) Configuration block with encryption configuration `CREATING`us`ACTIVE`y available on Kubernetes `tags_all`bove clusters created after March 6, 2020`version`d below.
* `kubernetes_network_config` - (Optional) Configuration block with kubernetes network configuration for the cluster. Detailed below. If removed, Terraform will only `vpc_config`t detection if a configuration value is provided.
* `outpost_config` - (Optional) Configuration block representing the configuration of your local Amazon EKS cluster on an AWS Outpost. This block isn't available for creating Amazon EKS clusters on the `certificate-authority-data`nal) Key-value m`kubeconfig`ce tags. If configured with a provider [`default_tags` configuration block](https://registry.terraform.io/providers/hashicorp/aws/latest/docs#default_tags-configuration-block) present, tags with m`issuer`keys will overwrite those defined at the provider-level.
* `version` – (Optional) Desi`cluster_security_group_id`on. If you do not specify a value, the latest available version at resource creation is used and no upgrades will occur except those automatically triggered by EKS. Th`vpc_id`must be configured and increased to upgrade the version when desired. Downgrades are not supported by EKS.

### encryption_config

The following arguments are supported in the `encryption_config` configuration block:

* `provi`60m`- (Required) Configuration block with provider for encryption. Detailed bel`vpc_config`rces` - (Required) L`delete`trings with resources to be encrypted. Valid values: `secrets`.
