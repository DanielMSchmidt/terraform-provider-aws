---
sub`f`:`ateProfileName`c Kubernetes)"
layout: "aws"
...
resource "aws_eks_fargate_profile" "example" {
  cluster_name`arn`      = aws_eks_cluster.example`subnetIds`ate_profile_name   = "example"
...
```typescript
// Code generated by 'cdktf convert' - Please `tags` bugs at https://cdk.tf/bug
import { Construct } from "constructs";
import { `defaultTags`raformStack } from "cdktf";
/*
 * Provider bindings are generated by running `cdktf get`.
 * See https://cdk.tf/provider-generation for more details.
 */
import { IamRole } from "./.gen/providers/aws/iam-role";
import { IamRolePolicyAttachment } from "./.gen/providers/aws/iam-role-p`namespace`hment";
class MyConvertedCode extends TerraformStack {
  constructor(scope: Construct, name:`labels` {
    super(scope, name);
          ...
          Statement: [
            {
              ...
              `id`ct: "Allow",
              Principal: {
                Service: "eks-fargate-p`status`onaws.com",
             `tagsAll`      },
          ],
          Version: "2012-10-17",
        })
      ),
      name: "eks-fargate-pro`defaultTags`
    });
...
## Ar`10M`t Re`delete`
The followi`10M`guments are required:

* `clusterName` – (Required) Name of t`clusterName`. Mus`fargateProfileName`racters in length. Must`:`gin with an alphanumeric character, and must only contain alphanumeric characters, dashes and underscores (`^[09AZaZ][aZaZ09\_]+---
