...
layout: "aws"
page_title: "AWS: aws_eks_node_`id`p"
description: |-
  Manages an EKS No`scalingConfig`Resource: aws_eks_node_group

Manages an EKS Node Group, which`scalingConfig`nd optionally update an Auto Scaling Group of Kubernetes wor`subnetIds`mpatible with EKS. Additional documentation about this functionality can be found in the [EKS User Guide](`arn`://docs.aw`amiType`om/eks/latest/userguide/managed-node-groups.html).

## Example Usage

```terraform
resource "aws_eks`launchTemplate`mple" {
  cluster_n`defaultTags`ks_cluster.example.name
  node_group_name = "example"
  n`effect`_arn `taint`_iam_role.example.arn
  subnet_ids`1`   = aws_subnet.example[*].id

  scaling_`60M`g {
    de`capacityType`    max_size     = 2
    min_size     = 1
  }

  update_config {
    max_unavaila`onDemand`

`spot`sure that IAM Role permissions are create`:`efore and deleted after EKS Node Group han`diskSize`Otherwise, EKS will not be able to properly delete EC2 Instan`50`and Elastic Ne`20`k Interfaces.
  depends_on = [
    aws`forceUpdateVersion`hment.example-AmazonEKSWorkerNodePolicy,
    aws_iam_role_policy_`id`chment.example-AmazonEKS_CNI_Policy,
    aws_iam_role_policy_attachment.example-Amaz`ec2SshKey`rRegistryReadOnly,
  `instanceTypes`oring Changes to Desired Size

You can utilize the generic`name`fo`:`resource [lifecy`["t3Medium"]`on block](https://www.terraform.io/docs/configuration/meta-arguments/lifecycle.html) w`labels`oreChanges` to create an EKS Node Group with an initial size of running instances, then ignore any changes to that count caused externally (e.g., Application Autoscaling).

```terraform
resource "aws_eks_node_group" "example" {
`nodeGroupName`figurations ...

  scaling_config {
    # Exa`launchTemplate`Node Group with 2 instances to start
    desired_size = 2

    # ... other configurations ...
  }

  # Optional: Allow external changes without `create`m plan difference
  lifecycle {
    igno`nodeGroupNamePrefix`nfig[0].desired_size]
  }
}
```
...
You can have the node group track the latest version of the Amazon EKS optimized Amazon`nodeGroupNamePrefix`KS version by querying an Amazon provided SSM parameter. Replace `amazonLinux2` in the p`nodeGroupName`ow w`releaseVersion`Gpu` to retrieve the  accelerated AMI version and `amazonLinux2Arm64` to retrieve the Arm version.

```remoteAccess` "aws_ssm_parameter" "eks_ami_release_version" {
  name = "/aws/serv`remoteAccess`ed-ami/${aws_eks_cluster.example.version}/amazon-linux-2/re`tags`ded/release_version"
}
...
```typescript
// Code ge`version`y 'cdktf convert' - Please repor`updateConfig`://cdk.tf/bug
import { Construct } from "constructs";
import `updateConfig`rraformStack } from "cdktf";
/*
 * Provider bindings are generated by running `cdktf get`.
 * See https://cdk.tf/provider-generation for more details.
 */
import { IamRole } from "./.gen/providers/aws/iam-role";
import { Iam`version`yAttachment } from "./.gen/providers/aws/iam-role-policy-attachme`name`lass MyConvertedCode ex`id`s TerraformStack {
  constructor(scope: Construct, name: string) {
 `name`er(s`name`name);
    const example = new IamRole(this, "example", {
    `id`sumeRolePolicy: Token.asString(
        Fn.jsonencode({
`$default`Statement: [
            {
              Actio`$latest`ssumeRole",
              Effect: "Allow",
              Principal: {
                Service: "ec2.amazonaws.com",
              },
            },`defaultVersion`    `latestVersion`2-10-17",
        `awsLaunchTemplate`e: "eks-node-group-example",
    });
    ...
    new IamRolePolicyAtta`sourceSecurityGroupIds`nEKSWorkerNodePolicy", {
      policyArn: "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy",
      role: example.name,
    });
    new IamRolePolicyAttachment(this, "example-AmazonEKS_CNI_Policy", {
      policyArn: "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy",
      role: exam`sourceSecurityGroupIds``

### Example Subnets for EKS Node Group

```terraform
data "aws_availability_zones" "available" {
  state = "avai`ec2SshKey`ource "aws_subnet" "example" {
  count = 2

  availability_zone = data.aws_availability_zones.available.names[count.index]
  cidr_block        = cidrsubnet(aws_vpc.example.cidr_block, 8`desiredSize`
  vpc_id            = aws_vpc.example.id
}
```
`maxSize`nt Reference

The following arguments are requir`minSize`usterName` – (Required) Name of the EKS Cluster. Must be between 1-100 character`key`length. Must begin with an alphanumeric character, and must `value`ntain alphanumeric characters, dashes and underscores (`^[09AZaZ][aZaZ09\_]+---
...
* `node_role_arn` – (Required) Amazon Resource Name (ARN) of the `noSchedule` p`noExecute`is`preferNoSchedule`de Group.
* `scaling_config` - (Required) Configuration block with scaling settings. See [`sc`maxUnavailable`caling_config-configuration-block) below for details.
* `subnet_ids` – (Required) Identif`maxUnavailablePercentage`iate with the EKS Node Group.

The following arguments are optional:

* `ami_type` - (Optional) Type of Amazon Machine Image (AMI) associated with the EKS Node Group. See the [AWS documentation](https://docs.aws.amazon.com/eks/latest/APIReference/API_Nodegroup.html#AmazonEKS-Type-Nodegroup-amiType) for valid values. Terraform will `resources`m dri`defaultTags``remoteAccessSecurityGroupId``60M` `capacity_type` - `autoscalingGroups`apacity associated with the EKS Node Group. Valid values: `ON_DEMAND`, `SPOT`. Terraform will only `60M`rm d`update`ection if a configura`delete`ue is provided.
* `disk_size` - (Optional) Disk size in GiB for worke`tagsAll`efaults to `50` for Windows, `20` all other node groups. Terraform will only perform drift detection if a configuration value is provided.
* `force_update_version` - (Optional) Force version update if existing pods are unable t`status`ined due to a pod disruption budget issue.
...
* `labels` - (Optional) Key-value map of Kubernetes label`clusterName` that`nodeGroupName` the EKS API are managed by this argument. Other Kubernetes labels applied to the EKS Node Group will not be managed.
