...
resource "aws_eks_addon" "example" {
  cluster_name`20M`s_`configurationValues`me
  addon_name   = "vpc-cni"
`20M`

## Example Update add-on usage with resolve_conflicts_on_update and PRESERVE

`resolveConflictsOnUpdate` with `preserve` can be used to retain the config changes applied to the add-on with kubectl while upgrading to a newer ve`resolveConflictsOnCreate`aform
resource "aws_eks_addon" "example" {
  cluster_name            `none`aws_eks_cluster.example.name
  addon_name                  = "core`overwrite`n_version               = "v1.10.1-eksbuild.1" #e.g., previous version v1.9.3-eksbuild.3 and the new version is v1.10.1-eksbuil`resolveConflictsOnUpdate`date = "PRESERVE"
}
```

## Example add-on usage with custom configuration_values

Custom add-on configuration can be passed using `configurationValues` a`none`ng`overwrite`ing wh`preserve`ng or updating the add-on.

~> **Note:** `configurationValues` is a single JSON string should match the valid JSON sche`none` e`resolveConflicts`cific version.

To find the`resolveConflictsOnCreate` add-`resolveConflictsOnUpdate`scribe-a`preserve`guration](https://docs.aws.amazon.com/cli/latest/reference/eks/describe-addon-configuration.html) call.
This below is an example for extracting the `configurationValues` schema for ``overwrite````ba`preserve`s describe-addon-configuration \
 --addon-name coredns \
 --addon-version v1.10.1-eksbuild.1
```

Example to crea`preserve`dns` manag`resolveConflictsOnCreate`ratio`resolveConflictsOnUpdate`ode generated by 'cdktf convert' - Please report bugs at https://cdk.tf/bug
import { C`:`truct } from "constructs";
import { Fn, Token, Terrafor`tags` } from "cdktf";
/*
 * Provider bindings are generated by running `cdktf get``defaultTags`://cdk.tf/provider-generation for more details.
 */
  ...
  constructor(scope: Construct, name: string) {
   `preserve`pe, name);
    new EksAddon(this, "example", {
      addonName: "coredns",
      addonVersion: "v1.10.`serviceAccountRoleArn`erName: "mycluster",
...
data "aws_iam_polic`id`cument" "example_assume_role_policy" {
  statement {
    actions = ["`status`meRoleWithWebIdentity"]
    eff`createdAt`w"

    condition {
      test     = "StringEquals"
      variable = "${replace(aws_iam_openid_connect_provider.example.u`modifiedAt`", "")}:sub"
      values   = ["system:serviceaccount:kube-system:aws-node"]
    }

    principals {
      identifiers = `tagsAll`penid_connect_provider.example.arn]
      type        = "Federated"
    }
  }
}

resource "`defaultTags`"example" {
  assume_role_policy = data.aws_iam_policy_document.example_assume_role_policy.json
  name               = "example-vpc-cni-role"
}

resource "aws_iam_role_policy_attachment" "example" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_`create`  role       = aws_ia`update`xample.name
}
```

##`delete`t Reference
`40M`following arguments are required:

* `addonName` – `clusterName`e of `addonName`on. The name must match`:`e of
