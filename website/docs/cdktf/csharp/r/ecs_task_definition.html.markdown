...
class MyConvertedCode : TerraformStack
{
    ...
    {
        new EcsTaskDefinition.EcsTaskDefinition(this, "service", new EcsTaskDefinitionConfig {
            ContainerDefinitions = Fn.Jsonencode(new [] { new Dictionary<string, object> {
                { "cpu", 10 },
                { "essential", true },
                { "image", "service-first" },
                { "memory", 512 },
                { "name", "first" },
                { "portMappings", new [] { new Struct {
                    ContainerPort = 80,
                    HostPort = 80
                } } }
            ...
            Family = "service",
            PlacementConstraints = new [] { new EcsTaskDefinitionPlacementConstraints {
                Expression = "attribute:ecs.availability-zone in [us-west-2a, us-west-2b]",
                Type = "memberOf"
            } },
            Volume = new [] { new EcsTaskDefinitionVolume {
...
class MyConvertedCode : TerraformStack
{
    public MyConvertedCode(Construct scope, string name) : base(scope, name)
    {
        new EcsTaskDefinition.EcsTaskDefinition(this, "test", new EcsTaskDefinitionConfig {
            ...
            ",
            Family = "test",
            InferenceAccelerator = new [] { new EcsTaskDefinitionInferenceAccelerator {
                DeviceName = "device_1",
